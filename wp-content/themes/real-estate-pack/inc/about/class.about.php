 ...
 * @since 1.0.0
 */
if (!class_exists('real_estate_pack_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class real_estate_pack_About
	{
		/**
		 ...
		 *
		 * @var real_estate_pack_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof real_estate_pack_About)) {
				self::$instance = new real_estate_pack_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<a href="' . esc_url('https://wpthemespace.com/product/real-estate-pack-pro/') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://wpthemespace.com/product/real-estate-pack-pro/') . '" target="_blank" >';
					echo esc_html('Real Estate Pack Pro Live Preview', 'real-estate-pack');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'real-estate-pack')
					)
				);
			}
		}
